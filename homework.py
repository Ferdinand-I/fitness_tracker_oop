...
class Running(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        # коэффициенты для расчёта калорий
        coeff_calorie_1 = 18
        coeff_calorie_2 = 20
...
def read_package(workout_type: str, data: list) -> Training:
    ...
    if workout_type == 'RUN':
        return workout_dict['RUN'](int(data[0]),
                                   float(data[1]),
                                   float(data[2]))
    elif workout_type == 'WLK':
        return workout_dict['WLK'](int(data[0]),
                                   float(data[1]),
                                   float(data[2]),
                                   float(data[3]))
    else:
        return workout_dict['SWM'](int(data[0]),
                                   float(data[1]),
                                   float(data[2]),
                                   float(data[3]),
                                   int(data[4]))
